...
export const getIllustrators = () => fetchJson<PersonOrPublisherGenerated[]>("illustrators.json");
...
export async function getIllustrator(illustratorId: string): Promise<PersonOrPublisherGenerated | undefined> {
  const illustrators = await getIllustrators();
  return illustrators.find((i) => i.id === illustratorId);
}
