...
**100作品規模の大量追加の場合**: ジャンル別に8〜10個のバッチ(各10〜15作品目標)に分け、各バッチの調査サブエージェントを並行(バックグラウンド)実行してよい。ただしJSONへの反映(手順3〜5)はサブエージェントの完了通知を受け取った順に**1バッチずつ逐次**行うこと(反映のたびに著者/イラストレーター/出版社のidセットが変わるため、並行反映すると重複idの検知ができない)。ジャンルを分けても同じ作品が複数バッチで候補に挙がることがあるが、`apply_batch`相当のスクリプトは既存work idと衝突する要素を自動スキップするだけなので実害はない。新規出版社は複数バッチから同一レーベルが別idで提案されることがあるため、適用前に既存出版社一覧および他バッチの提案と目視突き合わせて統合すること。
...
jsfdbのような大量タグ(1テーマ1〜数作品)にはせず、再利用可能な少数タグに絞る。1作品あたり平均4〜5テーマ程度が目安。新規作品を追加する際、既存タグで表現しきれない要素があれば`themes.json`にタグを追加してよい(スポーツ/ダークファンタジー/逆異世界/戦記/デスゲーム/終末・ポストアポカリプス/歴史IF/悪役令嬢/追放・パーティ解雇/スローライフなど、これまでも都度追加してきた)。
...
15作品(初回)→105作品(2026-07-31)→155作品(2026-08-01、受賞作50作品追加)→205作品(2026-08-01、デスゲームジャンル20作品+幅広いジャンル30作品を追加)→255作品(2026-08-01、幅広いジャンルの人気作50作品をさらに追加)→359作品(2026-08-01、悪役令嬢・追放・なろう系チート無双・学園ラブコメ・ミステリ・SF/ロボット・ホラー/オカルト・和風/中華風ファンタジー・スローライフ・往年の名作の10ジャンル別バッチで104作品を追加)。359作品時点で著者316・イラストレーター298・出版社56・テーマ42・アワード30。新規テーマとして「悪役令嬢」「追放・パーティ解雇」「スローライフ」を追加。

## 既知の未着手事項

- **新人賞 / それ以外の賞でのフィルター**: 受賞歴を「新人賞(デビュー契機の公募賞)」と「それ以外(このラノすごい!等の人気投票・ランキング)」に区別してフィルターできるようにする。awards.jsonに賞の種別を表すフィールドの追加が必要
- **表紙画像の未解決作品(12作品)**: 「陰陽ノ京」「境界の彼方」「中二病でも恋がしたい！」(以前から未解決)に加え、359作品化の際に「フォーチュン・クエスト」「ザンヤルマの剣士」「吸血鬼ハンター"D"」「宇宙皇子」「クレギオン」「ARIEL」「高機動幻想ガンパレード・マーチ」「Occultic;Nine -オカルティック・ナイン-」「名湯『異世界の湯』開拓記〜アラフォー温泉マニアの転生先は、のんびり温泉天国でした〜」が未解決(楽天ブックス・Koboどちらにも書誌なし、または絶版で確認できず)。他の書誌情報源を個別確認するか、`covers-cache.json`を手動で埋める
