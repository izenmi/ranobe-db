#!/usr/bin/env python3
"""候補タイトルを国立国会図書館サーチAPIで一括下調べする(mystery-db版)。

Wikipediaに記事がない小説が大半なので、書誌はこちらが主。probe.py(Wikipedia版)は
映像化の有無やシリーズ情報が欲しいときに補助的に使う。
...
def fetch(params, sleep, tries=4):
    ...
    for attempt in range(tries):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "mystery-db-probe/1.0"})
...
def main():
    ...
    def probe_one(i, cand):
        ...
        if hit is None:
            # ラノベは副題が長く「無職転生」と「無職転生〜異世界行ったら本気だす〜」のように
            # 長さが大きく違う。包含していれば(短い側が5文字以上なら)重複とみなす
            for k, wid in existing.items():
                if not k:
                    continue
                if (k in key and len(k) >= 5) or (key in k and len(key) >= 5):
                    hit = wid
                    break
        if hit:
