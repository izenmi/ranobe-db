...
//   2. Rakuten Kobo (Kobo/EbookSearch) — same app credentials. Covers small-press / web-novel
//      series that Rakuten doesn't stock in print. Kobo items have no ISBN field (itemNumber
//      instead), so these are cached with isbn: null and source: "kobo".
//   3. BOOK☆WALKER (HTML) — added 2026-08-03. KADOKAWA's own store, so it carries the 電撃文庫 /
//      MF文庫J / MFブックス / カドカワBOOKS / HJ NOVELS / 星海社 e-FICTIONS e-book editions that
//      Rakuten's catalog keeps missing. No public API, but both the search page and the product
//      pages are server-rendered: the search page links to https://bookwalker.jp/de<uuid>/ (and
//      returns HTTP 404 when there are zero hits, which is a clean "no results" signal), and each
//      product page carries <meta property="og:image"> plus a <title> of the form
...
// promoted items (searching "中二病でも恋がしたい 虎虎" surfaces business books), and manga
// adaptations of the same series rank above the novel. So a candidate is only accepted when its
// genre isn't マンガ（漫画）/実用/etc., an author name from authors.json appears in the page
// title, AND the normalized core title is contained in it — the same three-point check that was
// applied by hand for the honto.jp entries, where it caught 3 manga false-matches.
//
...
//
// --force re-fetches everything, including entries that were filled in by hand (the honto.jp
// ones), so prefer --retry-misses when you just want to have another go at the unresolved works:
// it only touches entries whose coverUrl is null and leaves every resolved entry alone.
...
function pickBestMatch(items, work, authorNames) {
  ...
  return (
    ...
    // Looser: the store's title merely contains our core title. Only trust this when the author
    // matches too, otherwise a shared subtitle would be enough to pull in an unrelated book.
    eligible.find(
      (it) =>
        core.length >= 3 &&
...
function pickBestKoboMatch(items, work, authorNames) {
  ...
  const core = normalize(coreTitle(work.title));
  const eligible = rankCandidates(
    items.filter((it) => !isPlaceholderImage(it.largeImageUrl)),
    (it) => it.title,
  );
  return (
    eligible.find((it) => normalize(it.title ?? "").startsWith(target)) ??
...
const BW_REJECTED_GENRES = ["マンガ（漫画）", "実用", "ゲーム攻略本", "雑誌", "写真集"];
